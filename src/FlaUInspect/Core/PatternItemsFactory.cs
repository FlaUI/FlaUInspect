...
public class PatternItemsFactory(AutomationBase? automationBase) {
	...
	private static IEnumerable<PatternItem> AddPatternSupportDetails(AutomationElement? element) {
		...
		var allSupportedPatterns = element.GetSupportedPatterns();
	...
	private static IEnumerable<PatternItem> AddTextPatternDetails(AutomationElement? element) {
		...
		var mixedValue = element.AutomationType == AutomationType.UIA2
		...
		var foreColor = GetTextAttribute<int>(element, pattern, TextAttributes.ForegroundColor, mixedValue, (x) => $"{Color.FromArgb(x)} ({x})");
		var backColor = GetTextAttribute<int>(element, pattern, TextAttributes.BackgroundColor, mixedValue, (x) => $"{Color.FromArgb(x)} ({x})");
		var fontName = GetTextAttribute<string>(element, pattern, TextAttributes.FontName, mixedValue, (x) => $"{x}");
		var fontSize = GetTextAttribute<double>(element, pattern, TextAttributes.FontSize, mixedValue, (x) => $"{x}");
		var fontWeight = GetTextAttribute<int>(element, pattern, TextAttributes.FontWeight, mixedValue, (x) => $"{x}");
	...
	private static IEnumerable<PatternItem> AddTablePatternDetails(AutomationElement? element) {
		...
		var pattern = element.Patterns.Table.Pattern;
	...
	private static IEnumerable<PatternItem> AddGridPatternPatternDetails(AutomationElement? element) {
		...
		var pattern = element.Patterns.Grid.Pattern;
	...
	private static IEnumerable<PatternItem> AddWindowPatternDetails(AutomationElement? element) {
		...
		var pattern = element.Patterns.Window.Pattern;
	...
	private static IEnumerable<PatternItem> AddInvokePatternDetails(AutomationElement? element) {
		...
		var pattern = element.Patterns.Invoke.Pattern;
		yield return new PatternItem("Invoke", "Invoke", pattern.Invoke);
	}

	private static string GetTextAttribute<T>(AutomationElement element, ITextPattern pattern, TextAttributeId textAttribute, object mixedValue, Func<T, string> func) {
